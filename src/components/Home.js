...
const Home = () => {
  ...
  return (
    <>
      <OnScope results={onScope[Math.floor(Math.random() * 10)]} genres={genres}/>
      <DataContainer results={trendingMovie} titulo={'Películas que son tendencia'} categoria={'trending'} tipo={'movie'}/>
      <DataContainer results={trendingTv} titulo={'Series que son tendencia'} categoria={'trending'} tipo={'tv'}/>
    </>
  );
}
